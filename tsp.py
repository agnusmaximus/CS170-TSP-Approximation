...
def read_input():
    ...
    colors = list(sys.stdin.readline())

    for i in range(n_nodes):
        for j, weight in enumerate(dist[i]):
...
constraints = lambda values: print("VALUES: " + str(values['weight'])) or True
...
p = TSP(G, objective = 'weight', start = 0, returnToStart=False, constraints=constraints) #, [optional] returnToStart={True}|False, constraints = ..., etc
